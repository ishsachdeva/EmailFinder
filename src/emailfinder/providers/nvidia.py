...
class NVIDIAReasoningProvider:
    def __init__(self, api_key=None, base_url=None, model=None, client=None, limiter=None, max_attempts=2, sleeper=time.sleep):
    ...
    def qualify_company(self, company, evidence, brief) -> QualificationOutput:
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                response = self.client.post(f"{self.base_url}/chat/completions", headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}, json={"model": self.model, "temperature": 0.1, "stream": False, "messages": [{"role": "system", "content": "You are an evidence-constrained B2B company classifier. Output valid JSON only."}, {"role": "user", "content": json.dumps(prompt)}]})
